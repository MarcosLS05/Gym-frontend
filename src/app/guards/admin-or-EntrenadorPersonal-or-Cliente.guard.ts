...
export class AdminOrEntrenadorPersonalOrClienteGuard implements CanActivate {
    ...
    canActivate(): Observable<boolean> {
        if (this.oSessionService.isSessionActive()) {
            ...
            return this.oUsuarioService.getUsuarioByEmail(email).pipe(
                map((data: IUsuario) => {
                    if (data.tipousuario.titulo === 'Administrador' || data.tipousuario.titulo === 'Entrenador Personal' || data.tipousuario.titulo === 'Cliente') {
                        return true;
                    } else {
